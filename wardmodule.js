...
wardapp.get("/v1/rooms/", verifyToken, async (req, res) => {
  try {
    const client = new MongoClient(uri);
    // Connect to the MongoDB server
    await client.connect();
    ...
    const db = client.db(dbName);
    const collection = db.collection(collectionName);
    const readRecord = await collection.find().toArray();
...
wardapp.post("/v1/admission/:wardnumber/:roomtype/", async (req, res) => {
  ...
  console.log(req.params);
  const { patientid, condition, admission_date, expected_discharge_date } =
    req.body;
  /**
     postman test for add: 
           {
  ...
  try {
    ...
    if (readPatient.length === 0) {
      ...
      const updatePatients = await collection.updateOne(
        ...
        { arrayFilters: [{ "r.room_number": room_number }] } //find room
      );
    }
    await client.close();
  } catch (err) {
...
wardapp.listen(8686, () =>
  console.log("Ward Management Server running on port 8686")
);
